...
#include <limits.h>
#include "defs.h"
#include "par_generation_utils.h"
#include <iostream>
...
void gen_RMAT_graph_MPI(graph_t* G)
{
    ...
    if(exp_2 == false){
      fprintf(stderr, "Error: size must be 2^n\n");
      exit(1);
    }
    ...
    edge_id_t edge_offset = G->m/G->nproc * G->rank;
    if (G->rank == G->nproc - 1) edge_offset += G->m % G->nproc;

    generate_unique_weights(G->m, weight, G->min_weight, G->max_weight, num_dir_edges, edge_offset);
